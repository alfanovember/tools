...
if ($install -eq $true) {p
	if ($link -eq $true) {
		...
		Start-Process -FilePath "msiexec.exe" -ArgumentList "/i $file RECEIVING_INDEXER=`"splunk.prod.dx:9997`" DEPLOYMENT_SERVER=`"vsacsrc01.prod.dx:8089`" WINEVENTLOG_SET_ENABLE=1 AGREETOLICENSE=Yes LAUNCHSPLUNK=0 /qb-" -Wait -Passthru
	}
}
